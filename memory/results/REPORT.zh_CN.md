...
分析也没有在 case 级别把 pair 分类、update
...
- 6.2 的分类是确定性文本判据给出的结构信号，不是语义标签；
  这是一项未保留原始产物与复现流程的一次性分析，也没有在 case
  级别把 pair 分类与 update 操作或答案正误关联，
  因此不能用来对具体错误答案做因果归因。
